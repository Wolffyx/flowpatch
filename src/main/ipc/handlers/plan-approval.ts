...
export function registerPlanApprovalHandlers(notifyRenderer: () => void): void {
  ...
  // Approve a plan
  ipcMain.handle('approvePlan', async (_e, payload: { approvalId: string; notes?: string }) => {
  ...
  // Reject a plan
  ipcMain.handle('rejectPlan', async (_e, payload: { approvalId: string; notes?: string }) => {
  ...
  ipcMain.handle('skipPlanApproval', async (_e, payload: { approvalId: string }) => {
