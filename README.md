...
Patchwork is a repo-linked Kanban app (Electron + React) that syncs issues/PRs/MRs from GitHub or GitLab and can run a local worker to turn "Ready" cards into branches + PRs/MRs using Claude Code or the OpenAI Codex CLI.
...
- Creates a per-repo `.patchwork/` workspace for config + local state (and adds `.patchwork/state/` to `.gitignore`).
...
- **Local workspace:** per-repo config (`.patchwork/config.yml`) plus local indexing/state used to build safe prompt/context bundles for the worker.
